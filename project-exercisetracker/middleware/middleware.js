...
middleware.createUser = (req, res, next) => {
	...
	User.create({ username })
		...
		.catch((err) => {
			...
			return res.json({
				error: 'Error when trying to create a new user',
			});
		});
};
...
middleware.createExercise = (req, res, next) => {
	...
	if (!description || !numericDuration || !validDate) {
		return res.json({
			error: 'Description (string), duration (number) and a valid date (YYYY-MM-DD) are required to create an Exercise',
			description,
			duration,
			unixDate,
		});
...
middleware.getFilteredExercisesByUserId = (req, res, next) => {
	...
	if ((from && !validDateStr(from)) || (to && !validDateStr(to))) {
		return res.json({
			error: `Invalid date given for from or to: FROM: ${
				from || 'N/A'
			}, TO: ${to || 'N/A'}`,
		});
	}
